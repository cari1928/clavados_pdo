...
//---------------------------------------------------------------------------------
    ...
    /**
     * Elimina un clavado
     * @param  String $cve_clavado ID del clavado a eliminar
     * @return void
     */
    public function deleteClavado($cve_clavado)
    {
        $sql  = "DELETE FROM clavado WHERE cve_clavado=:cve_clavado";
        $stmt = $this->conn->Prepare($sql);
        $stmt->bindParam(':cve_clavado', $cve_clavado, PDO::PARAM_STR);
        $stmt->execute();
    }

    /**
     * Realiza una consulta para obtener un clavado específico
     * @param  String $cve_clavado ID para obtener el clavado específico
     * @return array               Resultados del query realizado
     */
    public function getClavado($cve_clavado)
    {
        $clavados  = array();
        $statement = $this->conn->Prepare("select * from clavado where cve_clavado='" . $cve_clavado . "'");
        $statement->Execute();
        $clavados = $statement->FetchAll(PDO::FETCH_ASSOC);
